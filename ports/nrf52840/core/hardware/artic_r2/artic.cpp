...
void ArticTransceiver::send_packet(ArgosPacket const& packet, unsigned int total_bits, const ArgosMode mode)
{
	...
	// First 24-bit word should encode the total payload length in bits
	uint8_t total_length[3];
	total_length[0] = ((total_bits + num_tail_bits) >> 16);
	total_length[1] = ((total_bits + num_tail_bits) >> 8);
	total_length[2] = (total_bits + num_tail_bits);
    burst_access(XMEM, TX_PAYLOAD_ADDRESS, total_length, NULL, sizeof(total_length), false);

	// Discard the first 3 bytes (SYNC) which is generated by the transceiver -- the tail bits are not
    // part of the input buffer but get read out from XMEM by the transceiver
	const uint8_t *buffer = (const uint8_t *)packet.c_str();
    burst_access(XMEM, TX_PAYLOAD_ADDRESS + 1, (const uint8_t *)&buffer[3], NULL, (total_bits - 24) / 8, false);
