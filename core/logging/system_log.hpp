...
public:
	...
	static void create() {
		...
		try {
			...
			f = new CircularFile(main_filesystem, SystemLog::filename, LFS_O_RDONLY, SensorLog::MAX_SIZE);
		} catch (int e) {
			// Create a new log file if the file does not exist
			f = new CircularFile(main_filesystem, SystemLog::filename, LFS_O_WRONLY | LFS_O_CREAT, SensorLog::MAX_SIZE);
		}
		delete f;
	}

	static void write(void *entry) {
		CircularFile f(main_filesystem, SystemLog::filename, LFS_O_WRONLY, SensorLog::MAX_SIZE);
		f.write(entry, (lfs_size_t)sizeof(LogEntry));
	}

	static void read(void *entry, int index=0) {
		CircularFile f(main_filesystem, SystemLog::filename, LFS_O_RDONLY, SensorLog::MAX_SIZE);
		if (index != 0)
